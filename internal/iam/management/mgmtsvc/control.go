...
func (s *Control) CreateKey(ctx context.Context, p management.Principal) (management.Credential, error) {
	out := management.Credential{ID: uuid.NewString(), Expires: time.Now().Add(24 * time.Hour)}
...
func (s *Control) Delegate(ctx context.Context, p management.Principal, email, role string) (management.Delegated, error) {
	...
	out = management.Delegated{Key: uuid.NewString(), Secret: raw, Expires: time.Now().Add(24 * time.Hour)}
...
func (s *Control) CreateEnvironment(ctx context.Context, p management.Principal, project, name string) (string, error) {
	if !validID(project) || strings.TrimSpace(name) == "" {
		return "", errx.Validation("invalid request")
	}
	id := uuid.NewString()
